...
if __name__ == "__main__": 
    ...
    #cb added so the following import can work from docs
    pathfile = os.path.dirname(os.path.realpath(__file__)) 
    sys.path.insert(1,os.path.dirname(pathfile)+'/')
    try:
       p=os.path.dirname(pathfile)+'/mhw_stats/'
       os.makedirs(p)
    ...
    currdir=os.path.dirname(pathfile)+'/'
    ...
    plt.xlabel(mhwname + ' event number')
    plt.savefig(currdir+'mhw_stats/' + mhwname + '_list_byNumber.png', bbox_inches='tight', pad_inches=0.5, dpi=150)
    ...
    plt.title(r'Cumulative Intensity')
    plt.ylabel(r'[$^\circ$C$\times$days]')
    plt.savefig(currdir+'mhw_stats/' + mhwname + '_list_byDate.png', bbox_inches='tight', pad_inches=0.5, dpi=150)

    # Plot top 10 events

    # Maximum intensity
    outfile = open(currdir+'mhw_stats/' + mhwname + '_topTen_iMax.txt', 'w')
    ...
    outfile.close()
    plt.savefig(currdir+'mhw_stats/' + mhwname + '_topTen_iMax.png', bbox_inches='tight', pad_inches=0.5, dpi=150)

    # Mean intensity
    outfile = open(currdir+'mhw_stats/' + mhwname + '_topTen_iMean.txt', 'w')
    ...
    plt.legend(['SST', 'threshold', 'seasonal climatology'], loc=4)
    outfile.close()
    plt.savefig(currdir+'mhw_stats/' + mhwname + '_topTen_iMean.png', bbox_inches='tight', pad_inches=0.5, dpi=150)

    # Cumulative intensity
    outfile = open(currdir+'mhw_stats/' + mhwname + '_topTen_iCum.txt', 'w')
    ...
    plt.legend(['SST', 'threshold', 'seasonal climatology'], loc=4)
    outfile.close()
    plt.savefig(currdir+'mhw_stats/' + mhwname + '_topTen_iCum.png', bbox_inches='tight', pad_inches=0.5, dpi=150)

    # Duration
    outfile = open(currdir+'mhw_stats/' + mhwname + '_topTen_Dur.txt', 'w')
    ...
    plt.legend(['SST', 'threshold', 'seasonal climatology'], loc=4)
    outfile.close()
    plt.savefig(currdir+'mhw_stats/' + mhwname + '_topTen_Dur.png', bbox_inches='tight', pad_inches=0.5, dpi=150)
    ...
    plt.ylabel(r'[$^\circ$C$\times$days]')
    plt.grid()
    plt.savefig(currdir+'mhw_stats/' + mhwname + '_annualAverages_meanTrend.png', bbox_inches='tight', pad_inches=0.5, dpi=150)

    # Save results as text data
    outfile =currdir+ 'mhw_stats/' + mhwname + '_data'
